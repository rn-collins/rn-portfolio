...
function validate(p){
 ...
 for(const r of p.records){if(!r.dispositionBasis)throw Error(r.buildId+': no basis');const cleared=r.acquisitionDisposition==='CLEARED-NOT-SUBSTITUTED';if(r.externalSelected!==cleared||r.externalStaged!==false||r.rnFallbackReady!==true||r.canva?.generationAuthorized!==false)throw Error(r.buildId+': estate safety state');if(cleared){if(!r.visualCandidateSearch?.selection||r.visualCandidateSearch.selection.binaryPresent!==false||r.visualCandidateSearch.selection.binaryRepositoryPath!==null)throw Error(r.buildId+': selected metadata/binary truth boundary absent');validateCleared(r);}else{const s=r.visualCandidateSearch?.searched||[];if(!s.length||!s.some(c=>nonempty(c.nextStep)||nonempty(c.acquisitionNextStep)))throw Error(r.buildId+': HOLD lacks next route');}if(JSON.stringify(r).match(/"selectedForUse":\s*true|"stagedAssetPath":\s*"|"provenanceSidecarPath":\s*"/))throw Error(r.buildId+': hidden selection/staging');}
 return {records:100,clear:24,hold:76,fallback:100,selected:24,staged:0,canva:0};
}
