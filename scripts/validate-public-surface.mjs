...
for(const needle of ['aria-labelledby="asset-review-heading"','Canva-ready distribution','rn-100-builds-canva-ready.zip','inventory.json','README.md','loading="lazy"','Open official source page','Open exact candidate asset','Original HOLD fallback graphic','Open the full fallback SVG','Download SVG ↓','download={`${id}-original-rn-fallback.svg`}','fallbackSpec?.accessibility.alt','fallbackSpec.caption','External candidate formally selected','External asset staged','readOnly disabled','Final hook-image decision','Promoted after direct-fit review','Rollback:'])check(assetPanel.includes(needle),'asset review UI contract missing '+needle);
...
for(const needle of ['Canva-ready 100-cover distribution','26 live official source-context covers','74 RN-owned covers','rn-100-builds-canva-ready.zip','inventory.json','inventory.csv','README.md','PACKAGE-CHECKSUMS.json'])check(assetOperations.includes(needle),'asset operations Canva-package contract missing '+needle);
const canvaPackageDir='apps/web/public/100-builds/canva-package';
for(const file of ['rn-100-builds-canva-ready.zip','inventory.json','inventory.csv','README.md','SHA256SUMS','PACKAGE-CHECKSUMS.json']){
 const rel=canvaPackageDir+'/'+file;try{const st=fs.lstatSync(path.join(root,rel));check(st.isFile()&&!st.isSymbolicLink(),rel+' must be a regular non-symlink file')}catch{fail.push(rel+' missing')}
}
...
check(promotedIds.size===26&&promotionReview.rejectedBuilds.length===4,'final cover promotion ID sets drift');
...
for(const record of visualPackages.records){
 ...
 if(promotedIds.has(record.buildId)){
  ...
  check(/^\/media\/100-builds\/(?:production-source-variants|production-source-context|source-excerpts|replacement-production-covers)\/.+\.svg$/.test(record.liveCover?.assetUrl||''),'invalid live source cover path for '+record.buildId);
...
console.log('PASS: 100 IDs; 200 A/B routes; 44 build archives; 116 safe UTF-8 allowlist records; 451 canonical sitemap paths; 100 dossier asset previews, 26 promoted official source-context covers with preserved RN rollback, plus one canonical 100-row asset-operations surface and a checksum-verified 100-cover Canva-ready distribution with explicit release gates and 76 checksum-verified, accessible, self-contained original HOLD fallback previews; fail-closed source reader.');
