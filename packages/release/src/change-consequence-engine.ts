...
export function exploreChangeConsequences(input:ChangeInput):ChangeAssessment{
 ...
 const impacted:ImpactedNode[]=[...pathMap.entries()].map(([id,paths])=>{const n=byId.get(id)!;return{id,label:n.label,relation:(direct.has(id)?'registered-direct-use':'registered-transitive-dependency') as ImpactedNode['relation'],paths:paths.sort((a,b)=>a.length-b.length||a.join('>').localeCompare(b.join('>'))),basis:[...(basisMap.get(id)||[])].sort(),owner:n.owner}}).sort((a,b)=>Math.min(...a.paths.map(p=>p.length))-Math.min(...b.paths.map(p=>p.length))||a.id.localeCompare(b.id));
