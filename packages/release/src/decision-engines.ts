...
export const ESCALATION_ROUTER_ENGINE_VERSION='019.2.0';
export type EscalationSeverity='INFO'|'ATTENTION'|'URGENT'|'CRITICAL';
export type EscalationEvent={id:string;correlationKey:string;label:string;consequence:number;urgency:number;confidence:number;detectedAt:number;acknowledgedAt?:number;ownerRole:string;requiredAuthority:string;channels:string[];acknowledgementMinutes:number;suppressible:boolean;evidence:string};
...
export type RoutedEscalation={eventIds:string[];correlationKey:string;label:string;severity:EscalationSeverity;score:number;primary:string;currentTarget:string;backup:string;channel:string;deadline:number;acknowledgedAt:number|null;reason:string;state:'SUPPRESSED'|'AWAITING ACKNOWLEDGEMENT'|'ACKNOWLEDGED'|'ESCALATED'|'UNROUTABLE'};
export type EscalationRouteAssessment={status:'ROUTES READY'|'ESCALATION REQUIRED'|'ROUTING GAPS';routes:RoutedEscalation[];correlatedCount:number;suppressedCount:number;gaps:string[];inherited:{aiControls:AIUseMapAssessment['status'];handoffs:RegulatedHandoffAssessment['status'];friction:InstitutionalFrictionAssessment['status']};engineVersion:string};
export function assessEscalationRoutes(input:EscalationRouteInput):EscalationRouteAssessment{
 const ai=assessAIUseControls(input.aiUse),handoffs=assessRegulatedHandoffs(input.handoff),friction=assessInstitutionalFriction(input.friction);const responders=new Map(input.responders.map(r=>[r.id,r]));const grouped=new Map<string,EscalationEvent[]>();const gaps:string[]=[];
 for(const event of input.events){if(!event.correlationKey.trim()){gaps.push(`${event.id}: define a correlation key.`);continue}(grouped.get(event.correlationKey)||grouped.set(event.correlationKey,[]).get(event.correlationKey)!).push(event)}
 if(!input.events.length)gaps.push('Add at least one evidenced event before declaring routes ready.');
 const routes:RoutedEscalation[]=[];let correlatedCount=0,suppressedCount=0;
 for(const [key,events] of grouped){const lead=[...events].sort((a,b)=>(b.consequence+b.urgency+b.confidence)-(a.consequence+a.urgency+a.confidence))[0];correlatedCount+=Math.max(0,events.length-1);const score=Math.max(0,Math.min(100,Math.round(lead.consequence*.45+lead.urgency*.35+lead.confidence*.2)));const severity:EscalationSeverity=score>=85?'CRITICAL':score>=65?'URGENT':score>=40?'ATTENTION':'INFO';const primary=responders.get(lead.ownerRole);const backup=primary?responders.get(primary.backupId):undefined;const deadline=lead.detectedAt+lead.acknowledgementMinutes*60000;const acknowledgementValid=lead.acknowledgedAt!==undefined&&lead.acknowledgedAt>=lead.detectedAt&&lead.acknowledgedAt<=deadline;const primaryAuthorized=!!primary?.authority.toLowerCase().includes(lead.requiredAuthority.toLowerCase());let state:RoutedEscalation['state']='AWAITING ACKNOWLEDGEMENT';let target=primary;
  if(severity==='INFO'&&lead.suppressible){state='SUPPRESSED';target=undefined;suppressedCount++}
  else if(!primary||!primaryAuthorized){state='UNROUTABLE';target=undefined;gaps.push(`${lead.label}: the primary route lacks ${lead.requiredAuthority} authority.`)}
  else if(acknowledgementValid){state='ACKNOWLEDGED'}
  else if(input.now>deadline||!primary.available){state='ESCALATED';target=backup;const backupAuthorized=!!backup?.authority.toLowerCase().includes(lead.requiredAuthority.toLowerCase());const channelCompatible=!!backup?.channels.some(c=>lead.channels.includes(c));if(!backup?.available||!backupAuthorized||!channelCompatible){state='UNROUTABLE';target=undefined;gaps.push(`${lead.label}: escalation backup must be available, authorized for ${lead.requiredAuthority}, and share an approved channel.`)}}
  const channel=target?.channels.find(c=>lead.channels.includes(c))||'none';routes.push({eventIds:events.map(e=>e.id),correlationKey:key,label:lead.label,severity,score,primary:primary?.name||'UNASSIGNED',currentTarget:target?.name||'UNASSIGNED',backup:backup?.name||'UNASSIGNED',channel,deadline,acknowledgedAt:lead.acknowledgedAt??null,reason:`${severity}: consequence ${lead.consequence}, urgency ${lead.urgency}, confidence ${lead.confidence}; ${events.length} correlated event(s); requires ${lead.requiredAuthority} authority.`,state});
 }
 if(ai.status==='ESCALATE / DO NOT DEPLOY')gaps.push('Close inherited Build 012 consequence-control gaps.');if(handoffs.status!=='ACCOUNTABILITY INTACT')gaps.push('Close inherited Build 017 cross-organization handoff gaps.');if(friction.status==='PROCESS UNDEFINED')gaps.push('Close inherited Build 018 process-definition gaps.');
 const status:EscalationRouteAssessment['status']=gaps.length||routes.some(r=>r.state==='UNROUTABLE')?'ROUTING GAPS':routes.some(r=>r.state==='ESCALATED')?'ESCALATION REQUIRED':'ROUTES READY';return {status,routes,correlatedCount,suppressedCount,gaps,inherited:{aiControls:ai.status,handoffs:handoffs.status,friction:friction.status},engineVersion:ESCALATION_ROUTER_ENGINE_VERSION};
}
