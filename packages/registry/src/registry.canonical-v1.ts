...
export const builds:CanonicalBuild[]=legacyBuilds.map((legacy,index)=>{
 ...
 const is001=legacy.id==='001',is002=legacy.id==='002',active=is001||is002; const phase=Math.floor(index/10)+1;
 ...
 functional:{...legacy.functional,status:active?'Building':'Planned',url:is001?legacy.functional.url:is002?'/100-builds/002/a':null,summary:meta.job},
 visual:{...legacy.visual,status:active?'Building':'Planned',url:is001?legacy.visual.url:is002?'/100-builds/002/b':null,title:is001?legacy.visual.title:`${meta.title} — Visual Build`,concept:meta.bWeb,mechanism:meta.bWeb},
 ...
 public:{...(legacy.public??{}),plainPurpose:meta.job,comprehensionRange,evidenceStatus:is001?(legacy.public?.evidenceStatus??'researched'):is002?'researched':'planned',novicePath:legacy.public?.novicePath??meta.job,saveablePayload:legacy.public?.saveablePayload,canonVerdict:'KEEP'}};
});
