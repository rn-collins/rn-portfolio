...
export const builds:CanonicalBuild[]=legacyBuilds.map((legacy,index)=>{
 ...
 const released=['001','002','003'].includes(legacy.id); const inLab=['004','005','006','007','008','009','010','011','012','013'].includes(legacy.id); const active=released||inLab; const phase=Math.floor(index/10)+1;
