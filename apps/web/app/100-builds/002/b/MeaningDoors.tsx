...
export default function MeaningDoors(){
 const [active,setActive]=useState(0); const [auto,setAuto]=useState(true); const [open,setOpen]=useState(false);
 useEffect(()=>{if(!auto)return;const id=setInterval(()=>{setOpen(false);setTimeout(()=>setActive(x=>(x+1)%doors.length),250);setTimeout(()=>setOpen(true),520)},3600);setOpen(true);return()=>clearInterval(id)},[auto]);
 ...
 return <div className={s.stage}>
  <div className={s.top}><span>002-B / INTERACTIVE VISUAL BUILD</span><button onClick={()=>setAuto(x=>!x)}>{auto?'PAUSE':'PLAY'}</button></div>
  ...
  <div className={s.corridor}>
   <div className={s.doors}>{doors.map((x,i)=><button key={x.name} data-active={i===active} onClick={()=>{setAuto(false);setActive(i);setOpen(true)}}>{x.name}</button>)}</div>
   <div className={s.portal} data-open={open}><div className={s.door}><span>{d.name}</span></div><div className={s.output}><small>{drift}</small><p>{d.line}</p></div></div>
  </div>
