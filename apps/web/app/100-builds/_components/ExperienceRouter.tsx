...
export default function ExperienceRouter({children}:{children:ReactNode}){
 ...
 const context=<section className={s.variantContext} aria-label={`Build ${id}${variant.toUpperCase()} orientation`}><article><span>WHAT IT IS</span><p>{variant==='a'?'The working A prototype: use its controls to test the build question.':'The B experience: watch the shareable cut, inspect the motion argument, then use the richer web master.'}</p></article><article><span>WHY IT MATTERS</span><p>{build.public?.plainPurpose??build.description}</p></article><article><span>RN&apos;S ROLE</span><p>RN conceived the question, structured the evidence boundaries, designed the experience, implemented the prototype and documented its limits.</p></article><article><span>BOUNDARY</span><p>{boundary}</p></article></section>;
 return <>{context}{variant==='b'?<BExperience id={id}>{children}</BExperience>:children}</>;
}
