...
function readAllowed(file:string){if(!publicArchiveDocumentSet.has(file))notFound();const absolute=path.resolve(repoRoot,file);if(!absolute.startsWith(repoRoot+path.sep)||!fs.existsSync(absolute))notFound();const stat=fs.lstatSync(absolute);if(!stat.isFile()||stat.isSymbolicLink())notFound();const realRoot=fs.realpathSync(repoRoot);const real=fs.realpathSync(absolute);if(!real.startsWith(realRoot+path.sep))notFound();try{return new TextDecoder('utf-8',{fatal:true}).decode(fs.readFileSync(real))}catch{notFound()}}
