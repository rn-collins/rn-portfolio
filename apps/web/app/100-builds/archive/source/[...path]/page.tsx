...
function readAllowed(file:string){if(!publicArchiveDocumentSet.has(file))notFound();const absolute=path.resolve(repoRoot,file);if(!absolute.startsWith(repoRoot+path.sep))notFound();if(!fs.existsSync(absolute))return 'This generated record has not been materialized yet. Its canonical generator is retained in the archive and will replace this notice when generation completes.\n';return fs.readFileSync(absolute,'utf8');}
export function generateStaticParams(){return publicArchiveDocuments.map(file=>({path:file.split('/')}));}
export async function generateMetadata({params}:{params:Promise<{path:string[]}>}):Promise<Metadata>{const {path:parts}=await params;const file=sourcePath(parts);if(!publicArchiveDocumentSet.has(file))return {title:'Archive record not found'};const title=`Archive Record — ${file.split('/').at(-1)}`;const description=`Full allowlisted system-of-record file: ${file}`;return {title,description,alternates:{canonical:`/100-builds/archive/source/${file}`}};}
